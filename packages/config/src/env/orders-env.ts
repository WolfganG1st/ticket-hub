...
export const ordersEnvSchema = baseEnvSchema.extend({
  ...
  ORDERS_OUTBOX_BATCH_SIZE: z.coerce.number().int().positive().optional().default(50),
  ORDERS_OUTBOX_POLL_INTERVAL_MS: z.coerce.number().int().positive().optional().default(1000),
});
